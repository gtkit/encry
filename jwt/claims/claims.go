...
// Options claims 可选参数.
type Options func(*Claims)

// WithRole 设置角色.
func WithRole(role string) Options {
	return func(claims *Claims) {
		claims.Roles = append(claims.Roles, role)
	}
}

func WithRoles(roles ...string) Options {
	return func(claims *Claims) {
...
// WithPrv 设置权限.
func WithPrv(prv string) Options {
	return func(claims *Claims) {
		claims.Prv = prv
	}
}
...
func WithExpiresAt(expiresAt time.Duration) Options {
	return func(claims *Claims) {
...
func WithJwtID(jwtID string) Options {
	return func(claims *Claims) {
		claims.ID = jwtID
	}
}
